---
title: セルフホスティングの細道
description: VPS を使って、自由度が高く、低コストなインターネットサービスを安全に運用するためのガイド。
template: splash
head:
  - tag: title
...
**セルフホスティング**とは、自分で管理するサーバー上でアプリケーションやサービスを運用することです。  
Heroku や Vercel のようなマネージドサービスに任せるのではなく、自分でインフラを管理することで、**システム構成・コスト・データの管理方法**を自分で決められます。
...
一方、**[Coolify](https://coolify.io/)** は、自分の VPS 上に PaaS のような環境を構築できる **セルフホスト型の管理ツール** です。  
