...
- **パスワード再設定** — ログイン画面の Forgot password からリセット用メールを送れるようになります。SMTP を設定していないとメールは届きません。SMTP を設定する前にパスワードを忘れた場合は、[[guides/coolify/installation#パスワードを忘れた場合|インストール手順のトラブルシューティング]] を参照してください。
